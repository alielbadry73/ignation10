...
app.get('/', (req, res) => {
    const filePath = path.join(publicDir, 'backend_root_live.html');
    if (require('fs').existsSync(filePath)) return res.sendFile(filePath);
    // fallback to repo root version
    return res.sendFile(path.join(__dirname, '..', 'backend_root_live.html'));
});
...
app.post('/api/login', (req, res) => {
    ...
    console.log('Login attempt:', { identifier, password: password ? '***' : 'missing', userType });
    ...
    db.get(`SELECT name FROM sqlite_master WHERE type='table' AND name='users'`, (tableErr, tableRow) => {
        ...
        const tryStudentsTeachers = () => {
            getRowByIdentifier('students', identifier, (sErr, sRow) => {
                ...
                if (sRow) {
                    ...
                    if (stored === password) {
                        // migrate to bcrypt
                        migratePlaintextToBcrypt('students', sRow, password);
                        return sendSuccess(sRow, 'student');
                    }
                    return res.status(401).json({ success: false, message: 'Invalid credentials', error: 'Invalid credentials' });
                }
        ...
        if (tableRow && tableRow.name === 'users') {
            console.log('Users table exists, querying users table for:', identifier);
            ...
            getRowByIdentifier('users', identifier, (uErr, uRow) => {
                ...
                console.log('User found in users table:', uRow ? 'Yes' : 'No');
                if (uRow) {
                    console.log('User details:', { id: uRow.id, email: uRow.email, role: uRow.role, hasPassword: !!uRow.password });
                }
                if (uRow && uRow.password) {
                    return bcrypt.compare(password, uRow.password, (bErr, same) => {
                        ...
                        console.log('Password mismatch, trying students/teachers tables');
                        tryStudentsTeachers();
                    });
                }
                // No matching users row or no password -> fallback
                console.log('No user found or no password, trying students/teachers tables');
                tryStudentsTeachers();
            });
        } else {
...
app.post('/api/register', (req, res) => {
    ...
    const { username, email, password, fullName, userType, first_name, last_name, phone, parent_phone } = req.body;

    console.log('Registration attempt:', { email, first_name, last_name, userType });
    ...
    bcrypt.hash(derivedPassword, 10, (hErr, hash) => {
        ...
        const sql = `INSERT INTO ${table} (email, password, first_name, last_name, phone, parent_phone, role) VALUES (?, ?, ?, ?, ?, ?, ?)`;
        const params = [derivedEmail, hash, first_name || 'Unknown', last_name || 'User', phone || null, parent_phone || null, derivedUserType];
...
function ensureEnrollmentsSchema(cb) {
    db.all(`PRAGMA table_info('enrollments')`, (err, cols) => {
        ...
        const needed = [
            ...
            { name: 'is_active', sql: 'ALTER TABLE enrollments ADD COLUMN is_active INTEGER DEFAULT 1' },
...
app.post('/api/admin/revoke-access', authenticateJWT, requireAdmin, (req, res) => {
    ...
    ensureEnrollmentsSchema(() => {
        db.all(`PRAGMA table_info('enrollments')`, (eErr, cols) => {
            ...
            // If is_active column exists, set it to 0; otherwise set status='revoked' if status column exists
            db.all(`PRAGMA table_info('enrollments')`, (pErr, pCols) => {
                ...
                if (existingNames.includes('is_active')) {
                    const finalSql = enrollment_id ? `UPDATE enrollments SET is_active = 0 WHERE id = ?` : `UPDATE enrollments SET is_active = 0 WHERE ${studentCol} = ? AND ${courseCol} = ?`;
                    const finalParams = enrollment_id ? [enrollment_id] : [student_id, course_id];
                    db.run(finalSql, finalParams, function(err) {
                        if (err) { console.error('DB error revoking access:', err); return res.status(500).json({ success: false, error: 'Database error' }); }
                ...
                } else if (existingNames.includes('status')) {
...
// Default route
app.get('/', (req, res) => {
    res.sendFile(path.join(__dirname, '..', 'backend_root_live.html'));
});
