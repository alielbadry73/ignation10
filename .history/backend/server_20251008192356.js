...
app.get('/api/orders/status/:userId', authenticateJWT, (req, res) => {
    ...
    db.get('SELECT COUNT(*) as approved_orders FROM orders WHERE user_id = ? AND status = ?', [userId, 'approved'], (orderErr, orderRow) => {
        if (orderErr) {
            console.error('Error checking orders:', orderErr);
            return res.status(500).json({ error: 'Failed to check status' });
        }
        
        const approvedOrders = orderRow ? orderRow.approved_orders : 0;
        ...
        db.all('PRAGMA table_info(enrollments)', (pragmaErr, cols) => {
            if (pragmaErr) {
                ...
                return res.json({
                    status: approvedOrders > 0 ? 'approved' : 'pending',
            ...
            let enrollmentQuery;
            if (hasStudentId) {
                enrollmentQuery = 'SELECT COUNT(*) as active_enrollments FROM enrollments WHERE student_id = ? AND is_active = 1';
            } else if (hasUserId) {
                enrollmentQuery = 'SELECT COUNT(*) as active_enrollments FROM enrollments WHERE user_id = ? AND is_active = 1';
            } else {
    ...
    db.get(`SELECT * FROM orders WHERE id = ?`, [order_id], (err, order) => {
        ...
        db.run(updateSql, [now, adminId, order_id], function(uErr) {
            ...
            courses.forEach((course, index) => {
                ...
                if (courseId) {
                    db.run(`INSERT OR IGNORE INTO enrollments (student_id, course_id, granted_by_admin, created_at) VALUES (?, ?, ?, ?)`,
                        [studentId, courseId, adminId, now],
                        function(enrollErr) {
                            if (!enrollErr) enrollmentsCreated++;
                            
                            // Send response after last course
                    ...
                    res.json({ success: true, message: 'Order approved (no valid course IDs found)' });
                }
            });
        });
    });
