...
app.get('/api/orders/status/:userId', authenticateJWT, (req, res) => {
    ...
    // Check if user has any approved orders or active enrollments
    db.get(
        `SELECT 
            (SELECT COUNT(*) FROM orders WHERE user_id = ? AND status = 'approved') as approved_orders,
            (SELECT COUNT(*) FROM enrollments WHERE student_id = ? AND is_active = 1) as active_enrollments`,
        [userId, userId],
        (err, row) => {
            if (err) {
            ...
            const hasAccess = row.approved_orders > 0 || row.active_enrollments > 0;
            
            res.json({
                status: hasAccess ? 'approved' : 'pending',
                hasAccess: hasAccess,
                approvedOrders: row.approved_orders,
