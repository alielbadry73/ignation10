...
app.post('/api/register', (req, res) => {
    // Accept either legacy fields (username, fullName, userType) or frontend fields (first_name, last_name)
    const { username, email, password, fullName, userType, first_name, last_name } = req.body;
    ...
    const table = derivedUserType === 'teacher' ? 'teachers' : 'students';
    ...
    bcrypt.hash(derivedPassword, 10, (hErr, hash) => {
        ...
        db.run(
            `INSERT INTO ${table} (username, email, password, full_name, first_name, last_name) VALUES (?, ?, ?, ?, ?, ?)`,
            [derivedUsername, derivedEmail, hash, derivedFullName, first_name, last_name],
            function(err) {
                if (err) {
                ...
                // Return a simple user object and no token here (login can be performed immediately after)
                return res.json({ success: true, message: 'Registration successful', userId: this.lastID });
            }
        );
    });
});
...
app.get('/api/admin/students', authenticateJWT, requireAdmin, (req, res) => {
    safeAll('students', res, (err, rows) => {
        if (err) return; // safeAll already handled response on error
