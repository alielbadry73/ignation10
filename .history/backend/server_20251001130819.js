...
app.post('/api/register', async (req, res) => {
  const { firstName, lastName, email, password, phone, parentPhone } = req.body;
    ...
    db.get('SELECT * FROM users WHERE email = ?', [email], async (err, existingUser) => {
      ...
      db.run(
        `INSERT INTO users (first_name, last_name, email, password, phone, role) 
         VALUES (?, ?, ?, ?, ?, ?)`,
        [firstName, lastName, email.toLowerCase(), hashedPassword, phone, 'student'],
        function(err) {
          if (err) {
          ...
          if (parentPhone) {
            db.run(
              `INSERT INTO parents (first_name, last_name, email, phone, whatsapp_number) 
               VALUES (?, ?, ?, ?, ?)`,
              [`${firstName}'s Parent`, 'Guardian', `parent_${email}`, parentPhone, parentPhone],
              function(parentErr) {
...
app.get('/api/my-enrollments', authenticateToken, (req, res) => {
  const query = `
    SELECT c.id, c.title, c.slug, c.instructor, e.enrolled_at, e.status
    FROM enrollments e
