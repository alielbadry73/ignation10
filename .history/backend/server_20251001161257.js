    ...
    db.get('SELECT * FROM users WHERE email = ?', [email], async (err, existingUser) => {
      ...
      db.run(
        `INSERT INTO users (first_name, last_name, email, password, phone, role) 
         VALUES (?, ?, ?, ?, ?, ?)`,
        [first_name, last_name, email.toLowerCase(), hashedPassword, phone, 'student'],
        function(err) {
          if (err) {
      ...
      courses.forEach(course => {
        db.run(orderItemsQuery, [orderId, course.id, course.price], (err) => {
          if (err) {
            ...
            // Try without quantity column
            const fallbackQuery = 'INSERT INTO order_items (order_id, course_id, price) VALUES (?, ?, ?)';
            db.run(fallbackQuery, [orderId, course.id, course.price], (fallbackErr) => {
              if (fallbackErr) console.error('Fallback also failed:', fallbackErr);
            });
          }
        });
      });
    ...
    db.run('UPDATE orders SET status = ? WHERE id = ?', ['completed', orderId], (err) => {
      ...
      db.all('SELECT course_id FROM order_items WHERE order_id = ?', [orderId], (err, items) => {
        ...
        if (items.length === 0) {
          ...
          return res.status(400).json({ error: 'No courses found in this order' });
        }
