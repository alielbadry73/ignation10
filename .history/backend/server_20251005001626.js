...
db.serialize(() => {
    // Update students table to match expected schema
    db.run(`ALTER TABLE students ADD COLUMN username TEXT`, (err) => {
        if (err && !err.message.includes('duplicate column name')) {
            console.error('Error adding username column:', err);
        }
    });
    db.run(`UPDATE students SET username = email WHERE username IS NULL`);
    
    // Create teachers table with correct schema if it doesn't exist
    ...
    // Copy data from old teachers table if it exists
    db.run(`INSERT OR IGNORE INTO teachers_new (id, email, full_name, created_at) 
            SELECT id, email, first_name || ' ' || last_name, created_at FROM teachers`);
    
    // Drop old table and rename new one
    db.run(`DROP TABLE IF EXISTS teachers`);
    db.run(`ALTER TABLE teachers_new RENAME TO teachers`);
