...
function seedDatabase() {
  ...
  db.get("SELECT COUNT(*) as count FROM users", (err, row) => {
    ...
    const seedData = `
      ...
      -- Insert default teacher for demo purposes (ID: 1)
      INSERT OR IGNORE INTO teachers (id, first_name, last_name, email, subject, experience_years)
      VALUES (1, 'Demo', 'Teacher', 'teacher@ignation.com', 'mathematics', 10);
...
// Create a new quiz
app.post('/api/teacher/quizzes', (req, res) => {
  const teacherId = 1; // Default teacher ID for demo
  const { subject, title, description, duration, total_questions, total_points } = req.body;
...
// Update a quiz
app.put('/api/teacher/quizzes/:id', (req, res) => {
...
// Delete a quiz
app.delete('/api/teacher/quizzes/:id', (req, res) => {
  const quizId = req.params.id;
  const teacherId = 1; // Default teacher ID for demo
...
app.get('/api/teacher/assignment-submissions', (req, res) => {
  const teacherId = 1; // Default teacher ID for demo
...
app.put('/api/teacher/grade-assignment/:id', (req, res) => {
...
// Get dashboard counts for course page
app.get('/api/dashboard/counts', (req, res) => {
  ...
  const query = `
    SELECT 
      (SELECT COUNT(*) FROM teacher_assignments WHERE subject = ? AND is_active = 1) as assignments_available,
      (SELECT COUNT(*) FROM teacher_exams WHERE subject = ? AND is_active = 1) as exams_available,
      (SELECT COUNT(*) FROM teacher_quizzes WHERE subject = ? AND is_active = 1) as quizzes_available,
      (SELECT COUNT(*) FROM assignment_submissions WHERE assignment_id IN (SELECT id FROM teacher_assignments WHERE subject = ? AND is_active = 1)) as assignments_completed,
      (SELECT COUNT(*) FROM quiz_submissions WHERE quiz_id IN (SELECT id FROM teacher_quizzes WHERE subject = ? AND is_active = 1)) as quizzes_completed,
      0 as exams_completed
  `;
  
  db.get(query, [subject, subject, subject, subject, subject], (err, row) => {
    if (err) {
      console.error('Error getting dashboard counts:', err);
      return res.status(500).json({ error: 'Database error' });
    }
    
    // Calculate pending counts (available - completed)
    const assignmentsPending = Math.max(0, row.assignments_available - row.assignments_completed);
    const examsPending = Math.max(0, row.exams_available - row.exams_completed);
    const quizzesPending = Math.max(0, row.quizzes_available - row.quizzes_completed);
    
    res.json({
      assignments: {
        available: row.assignments_available,
        completed: row.assignments_completed,
        pending: assignmentsPending
      },
      exams: {
        available: row.exams_available,
        completed: row.exams_completed,
        pending: examsPending
      },
      quizzes: {
        available: row.quizzes_available,
        completed: row.quizzes_completed,
