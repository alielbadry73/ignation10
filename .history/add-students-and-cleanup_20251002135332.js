...
function addStudentsAndCleanup() {
    console.log('Starting student management...');
    ...
    db.all("SELECT id, first_name, last_name, email FROM users WHERE role = 'student'", (err, rows) => {
        ...
        db.run("DELETE FROM users WHERE role = 'student'", function(err) {
            ...
            // Add Ahmed Zaher
            db.run(`
                INSERT INTO users (first_name, last_name, email, password, role, is_active, points, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))
            `, ['Ahmed', 'Zaher', 'ahmed.zaher@example.com', 'password123', 'student', 1, 0], function(err) {
                if (err) {
                ...
                db.run(`
                    INSERT INTO users (first_name, last_name, email, password, role, is_active, points, created_at, updated_at)
                    VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))
                `, ['Saad', 'Samir', 'saad.samir@example.com', 'password123', 'student', 1, 0], function(err) {
                    if (err) {
                    ...
                    db.all("SELECT id, first_name, last_name, email, points FROM users WHERE role = 'student'", (err, finalRows) => {
                        if (err) {
                        ...
                        finalRows.forEach(row => {
                            console.log(`- ${row.first_name} ${row.last_name} (${row.email}) - ID: ${row.id} - Points: ${row.points}`);
                        });
