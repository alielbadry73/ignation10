...
function countAllNotifications() {
    try {
        ...
        const allAssignments = [...assignments, ...teacherAssignments];
...
function getAllNotifications() {
    ...
    allQuizzes.filter(q => q.published !== false && !q.completed).forEach(quiz => {
        const timeAgo = quiz.publishedDate ? getTimeAgo(new Date(quiz.publishedDate)) : 'Recently';
        notifications.push({
            type: 'quiz',
    ...
    allAssignments.filter(a => !a.completed && a.published !== false).forEach(assignment => {
        ...
        const timeAgo = assignment.publishedDate ? getTimeAgo(new Date(assignment.publishedDate)) : 'Recently';
        notifications.push({
            type: 'assignment',
    ...
    allExams.filter(e => e.published !== false && !e.completed).forEach(exam => {
        ...
        const timeAgo = exam.publishedDate ? getTimeAgo(new Date(exam.publishedDate)) : 'Recently';
        notifications.push({
            type: 'exam',
